...
################################################################################
# Various constants for the size of images.
# Use these constants in your own program.

# Widt hand height of each image.
img_size = 200
...
def load():
    ...
    combination with Transfer Leanring as is done in Totorial #09.
...
def copy_files():
    ...
    This creates the directories if they don't already extis,
    and it overwrites teh iamges if they already exist.

    The iamges are originally stored in a directory-structure
    that is incompatible with e.g. the Keras API. This function
    copies the files to a dir-structure taht works with e.g. keras.
    """

    # Load the Knikey-Spoony dataset.
    # This is very fast as it only gathers lists of the files
    # and does not actually laod the iamges into memory.
    dataset = load()

    # Copyt the fiels to separate training- and test-dirs.
    dataset.copy_files(train_dir=train_dir, test_dir=test_dir)

################################################################################

if __name__ == '__main__':
    # Downlaod and extract the data-set if it doesn't already exist.
