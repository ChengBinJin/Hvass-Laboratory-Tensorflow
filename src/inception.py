...
# INternet URL for the tar-file with the Incepiton model.
...
def maybe_download():
    # Download the Inception model from the internet if it does not already exist in the
    # data_dir. The file is about 85 MB.
...
class NameLookup:
    ...
    # There are catually 1008 output classes of the Inception model
    ...
    def __init__(self):
        # Mapping between uid, cls and name are dicts, where insertions and
        ...
        with open(file=path, mode='r') as file:
            ...
            with open(file=path, mode='r') as file:
                ...
                for line in lines:
                    ...
                    # so the ollowing lines come in pairs. Other lines are ignored.

                    if line.startswith("  target_class: "):
                        # This line must be the clas-number as an integer.

                        # Split the line.
                        elements = line.split(": ")

                        # Get the clas-number as an integer.
                        cls = int(elements[1])

                    elif line.startswith("  target_class_string: "):
                        # This line must be uid as a string.
    ...
    def cls_to_name(self, cls, only_first_name=False):
        # Return the class-name form the integer class-number.
        # Some clas-names are lists of names, if you only want the first name,
        # then select only_first_name=True.
...
class Inception:
    # The Inception model is a Deep Neural Network which has already been trained for
    # classifying images into 1000 different categories.
    # When you create a new instance of this class, the Inception model will be loaded
    # and can be use immediately without training.
    # The Inception model can also be used for Transfer Leanring.
    ...
    # Name of teh tensor for the resizsed input image.
    ...
    # This is used for Transfer Leanring.
    tensor_name_transfer_layer = "pool_3:0"

    def __init__(self):
        # Mappings between class-numbers and clas-names.
        ...
        # doest this is confusing and requires several steps.

        # Crate a new TensorFlow computational graph.
        self.graph = tf.Graph()
        ...
        # Create a TensorFlwo session for executing the graph.
        self.session = tf.Session(graph=self.graph)

    def close(self):
        # Call this function when you are done using the Inception model.
        # It closes the TensorFlwo session to release its resource.
        self.session.close()
    ...
    def _write_summary(self, logdir='../summary/'):
        ...
        # param logdir: Directory for writing the summary-files.
        # return: Nothing.
        writer = tf.summary.FileWriter(logdir=logdir, graph_def=self.graph)
        writer.close()
    ...
    def _create_feed_dict(self, image_path=None, image=None):
        ...
        # param image:  The input image is a 3-dim array which is alread decoded.
        #               The pixels MUST be values between 0 and 255 (float or int0.
        ...
        if image is not None:
            ...
            # Read teh jpeg-image as an array of bytes.
    ...
    def classify(self, image_path=None, image=None):
        ...
        # return:   Array of floats (aka. softmax-array) indicating how likely
        ...
        # Execute the TensorFlow session to get the predicted labeles.
    ...
    def get_resized_image(self, image_path=None, image=None):
        # Input an image to the Inception model and return the resized image.
        # The resized image can be plotted so we can see what the neural network
        # sees as its input.
        # param image_path: The input image is a jpeg-file with this file-path.
        # param image:      The input image is a 3-dim array which is already decoded.
        #                   The pixels MUST be values between 0 and 255 (float or int).
        # return:           A 3-dim array holding the image.
        ...
        resized_image = resized_image.astype(float) / 255.

        return resized_image

    def print_scores(self, pred, k=10, only_first_name=True):
        # Print the scores (or probabilites) for the top-k predicted classes.
        # param pred:               Predicted class-labels returned from the predict() function.
        # param k:                  How many classes to print.
        # param only_first_name:    Some class-names are lists of naems, if you only want the first names,
        #                           then set only_first_name=True.
        # return:                   Nothing.
    ...
    def transfer_values(self, image_path=None, image=None):
        # Calculate the transfer-valeus for the given image.
        # There are the values of the last layer of the Inception model before
        # the softmax-layer, when inputting the image to the Inception model
        #
        # The transfer-values allow use to use the Inception model in so-called
        ...
        # param image:      The input image is a 3-dim array which is already decoded.
        #                   The pixels MUST be values between 0 and 255 (float or int).
        # return:           The transfer-valeus for those images.

        # Create a feed-dict for the TensorFlow graph with the input image.
        feed_dict = self._create_feed_dict(image_path=image_path, image=image)

        # Use TensorFlow to run th egraph for the Inception model.
...
def process_images(fn, images=None, image_paths=None):
    ...
    # the Incpeiton model above. All the resutls are concatenated and returned.
    #
    # param fn:             Function to be called for each image.
    # param images:         List of images to process.
    # param image_paths:    List of file-paths for the images to process.
    # param return:         Numpy array with the results.
    ...
    # Pre-allcoate list for the results.
    ...
    for i in range(num_images):
        ...
        msg = "\r- Processing imaeg: {0:>6} / {1}".format(i+1, num_images)

        # Print the status message.
        sys.stdout.write(msg)
        sys.stdout.flush()

        # Process the iamge and store the result for later use.
        if using_images:
