...
# 3)    Get random batches of training dat using data.random_batch()
...
#
# Published under the MIT License. See the file LICENE for details.
#
...
class MNIST:
    ...
    # Number of colour channels for the iamges: 1 channel for gray-scale.
    num_channels = 1
    ...
    def __init__(self, data_dir="../data/MNIST/"):
        ...
        self.x_train = x_train[0:self.num_train] / 255.
        self.x_val = x_train[self.num_train:] / 255.
        self.y_train_cls = y_train_cls[0:self.num_train]
        self.y_val_cls = y_train_cls[self.num_train:]

        # Download / load the test-set.
        self.x_test = self._load_images(filename=filename_x_test) / 255.
    ...
    def _load_data(self, filename, offset):
        ...
        # Downlaod the file from the internet if it does not exist locally.
    ...
    def _load_images(self, filename):
        ...
        # Reshape to 2-dim array with shape (num_images, img_sizse_flat).
    ...
    def _load_cls(self, filename):
        """
        Load class-numbers from teh given file.
        Automatically downloads the file if it does not exist locally.
    ...
    def random_batch(self, batch_size=32):
        ...
        :param batch_size: Number of images in teh batch.
